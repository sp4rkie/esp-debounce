...
#   CHIP = esp8266 #(default)
    ...
    bell.ino.cpp_CFLAGS = -DESP_1 -fpermissive
else ifeq ($(MAKECMDGOALS),esp32-0)
    CHIP = esp32
    BOARD = d1_mini32
    UPLOAD_PORT = /dev/ttyUSB3
    OTA_ADDR = $(MAKECMDGOALS)
    bell.ino.cpp_CFLAGS = -DESP32_0 -fpermissive
else
...
#SKETCH = $(HOME)/Arduino/pq/pq.ino                                        # SKETCH not needed if in current dir
#SKETCH = $(HOME)/Arduino/ota/ota.ino                                      # for initial OTA acivate only
#EXCLUDE_DIRS = $(HOME)/Arduino/libraries/TFT_eSPI
#BUILD_DIR = /tmp/mkESP                                                    # default
#BUILD_EXTRA_FLAGS =                                                       # overrides all (because it gets last)
